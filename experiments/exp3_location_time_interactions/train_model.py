...
def run_model_pipeline():
    ...
    if "time_slot" not in train_df.columns:
        train_df["time_slot"] = train_df["hour"] * 4 + (train_df["minute"] // 15)
        test_df["time_slot"] = test_df["hour"] * 4 + (test_df["minute"] // 15)

    print("time_slot nunique:", train_df["time_slot"].nunique())

    train_df["geo4_time_slot"] = train_df["geo4"].astype(str) + "_" + train_df["time_slot"].astype(str)
    train_df["geo5_time_slot"] = train_df["geo5"].astype(str) + "_" + train_df["time_slot"].astype(str)
    train_df["RoadType_time_slot"] = train_df["RoadType"].astype(str) + "_" + train_df["time_slot"].astype(str)

    test_df["geo4_time_slot"] = test_df["geo4"].astype(str) + "_" + test_df["time_slot"].astype(str)
    test_df["geo5_time_slot"] = test_df["geo5"].astype(str) + "_" + test_df["time_slot"].astype(str)
    test_df["RoadType_time_slot"] = test_df["RoadType"].astype(str) + "_" + test_df["time_slot"].astype(str)

    print("train_df['geo4_time_slot'].nunique():", train_df["geo4_time_slot"].nunique())
    print("train_df['geo5_time_slot'].nunique():", train_df["geo5_time_slot"].nunique())
    print("train_df['RoadType_time_slot'].nunique():", train_df["RoadType_time_slot"].nunique())

    train_df.drop(columns=["time_slot"], inplace=True)
    test_df.drop(columns=["time_slot"], inplace=True)
    ...
    submission.to_csv('submission_exp3_location_time.csv', index=False)
    print("💾 Finished! Smoothed 'submission_exp3_location_time.csv' generated cleanly.")
    
    # Export feature importance
    importance = model.feature_importance(importance_type='split')
    feat_imp = pd.DataFrame({'feature': model.feature_name(), 'importance': importance})
    feat_imp = feat_imp.sort_values(by='importance', ascending=False)
    feat_imp.to_csv('feature_importance_exp3.csv', index=False)
