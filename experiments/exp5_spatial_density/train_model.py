...
def run_model_pipeline():
    ...
    print("🌍 Generating spatial density features...")
    for df in [train_df, test_df]:
        df['geo5_density_ratio'] = df['geo5_freq'] / (df['geo4_freq'] + 1)
        df['geo6_density_ratio'] = df['geo6_freq'] / (df['geo5_freq'] + 1)
        df['geo4_density_ratio'] = df['geo4_freq'] / (df['geohash_freq'] + 1)
        df['local_density_score'] = (df['geo4_freq'] + df['geo5_freq'] + df['geo6_freq']) / 3

    density_cols = ['geo5_density_ratio', 'geo6_density_ratio', 'geo4_density_ratio', 'local_density_score']
    ...
    submission.to_csv('submission_exp5_spatial_density.csv', index=False)
    print("💾 Finished! Smoothed 'submission_exp5_spatial_density.csv' generated cleanly.")
    
    # Export feature importance
    importance = model.feature_importance(importance_type='split')
    feat_imp = pd.DataFrame({'feature': model.feature_name(), 'importance': importance})
    feat_imp = feat_imp.sort_values(by='importance', ascending=False)
    feat_imp.to_csv('feature_importance_exp5.csv', index=False)
